...
<h1>
	...
	<br/>
	<?php print $vs_titre."<br/>".$vs_label; ?>
</h1>

<table id="table">
<?php
//var_dump($va_results);die();
	foreach($va_results as $index => $row) {
		if($index == 0) {
			...
			print "<TR>";
			foreach($row as $col => $cell) {
				// Pour la colonne 14, volume en m3, on remplace la virgule par un point
				if($col == 14) {
					print "<td>".str_replace(",",".",$cell)."</td>\n";	 
				} else {
					print "<td>".$cell."</td>\n";
				}
    ...
    $('#table').DataTable(
	    {
            ...
            "buttons": [
				...
				{
					extend: 'excelHtml5',
					title: "<?php print str_replace('"', '\"',$vs_label); ?>"
				},
				{
					extend: "print",
					title: function(){
						return "<?php print "Inventaire des contenants".str_replace('"', '\"',$vs_label); ?>"
					},
