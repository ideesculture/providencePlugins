...
function getStorageLocationID($ps_location, $vn_loc_type_id, $options = [])
{
	...
	if (!$t_loc->load(array('idno' => $ps_location, "deleted" => 0))) {
		if ($VERBOSE) {
			print "CREATING LOCATION $ps_location\n";
		}
		// insert location
		...
		if ($t_loc->numErrors()) {
			print "ERROR INSERTING location ($ps_location): " . join('; ', $t_loc->getErrors()) . "\n";
			die();
		}
...
function getMovementID($ps_mov, $vn_loc_type_id, $options = [])
{
	...
	if (!$t_mov->load(array('idno' => $ps_mov, "deleted" => 0))) {
		if ($VERBOSE) {
			print "CREATING MOVEMENT $ps_mov\n";
		}
		// insert location
...
function getObjectID($ps_object, $ps_object_idno, $pn_object_type_id)
{
	...
	$t_label = $t_obj->getLabelTableInstance();
	$e_search = new ObjectSearch();
	$qr_results = $e_search->search($ps_object);

	if ($qr_results->numHits() == 0) {
		if ($VERBOSE) print "CREATING Object {$ps_object}\n";
		// insert occurrence
		$t_obj->setMode(ACCESS_WRITE);
		$t_obj->set('locale_id', $pn_locale_id);
		$t_obj->set('type_id', $pn_object_type_id);
		$t_obj->set('access', 1);
		$t_obj->set('status', 2);
		$t_obj->set('idno', $ps_object_idno);

		$t_obj->addAttribute(array(
			'locale_id' => $pn_locale_id,
			'name' => $ps_object
		), 'name');
		...
		if ($t_obj->numErrors()) {
			print "ERROR INSERTING Object ($ps_object): " . join('; ', $t_obj->getErrors()) . "\n";
			return null;
		}
		$t_obj->addLabel(array(
			'name' => $ps_object
		), $pn_locale_id, null, true);
		if ($t_obj->numErrors()) {
			print "ERROR ADDING LABEL Object ($ps_object): " . join('; ', $t_obj->getErrors()) . "\n";
			return null;
		}

		$vn_object_id = $t_obj->getPrimaryKey();
	} else {
		while ($qr_results->nextHit()) {
			$vn_object_id = $qr_results->get('ca_objects.object_id');
			if ($VERBOSE) print "\tFound Entity {$ps_object}\n";
			break 1;
		}
	}
	return $vn_object_id;
...
// ----------------------------------------------------------------------
function getEntityID($ps_forename, $ps_surname, $pn_type_id)
{
	...
	$qr_results = $e_search->search("ca_entities.preferred_labels:" . trim($ps_surname) . " " . trim($ps_forename));
	if ($qr_results->numHits() == 0) {
		$qr_results = $e_search->search($ps_surname . ', ' . $ps_forename);
	}
	...
	if ($qr_results->numHits() == 0) {
		...
		while ($qr_results->nextHit()) {
			...
			$vn_entity_id = $qr_results->get('ca_entities.entity_id');
...
function getEntityIDByIdno($ps_forename, $ps_surname, $ps_entity_idno, $pn_entity_type_id)
{
	...
	if (!$t_entity->load(array('idno' => $ps_entity_idno, 'deleted' => 0))) {
		...
		$t_entity->set('idno', $ps_entity_idno);
		$t_entity->insert();
